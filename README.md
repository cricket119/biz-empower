...
已接入 Supabase 免费数据库（`reports` 表）：

1. 员工填日报点「保存」→ 自动同步云端（需先设置姓名；离线保存的会在下次打开页面时自动补传最近3天）
2. 主管打开「管理视角 → 团队看板」→ **实时**看到全团队排行、提交率、趋势，可点「↻ 刷新」
3. 安全策略：前端密钥只能读取/写入日报，**不能删除**；删数据请登录 Supabase 后台操作

备用方式（云端故障时）：微信群日报文本粘贴导入、JSON 文件导入，入口在「数据汇总」页。
